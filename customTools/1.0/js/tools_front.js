	...
	(function() {
	    ...
	    // Add button to trigger UT tools
		   if(!$("#custom-tools-accordion").length>0){
		    ...
		    setTimeout(function () {
				$(".addUTTools").get(0).scrollIntoView();
            }, 300);
	    });
	    ...
	    $("#wiki_page_submit").click(function (){
	    	var templateClass = $(iframeID).contents().find("#ut-template-front").attr("class");
			if($.inArray(templateClass, nogridThemeArray) > -1){
				$(iframeID).contents().find("#ut-modules-grid").remove();
			}
	    });
...
///////////////////////////////
	...
	// Styles and code to be applied to TinyMCE editor
		function addStyletoFrontIframe(){
			if(!$(iframeID).contents().find("body").hasClass('hasStyle')){
				...
				// $("#ut_tools").get(0).scrollIntoView();
				$(iframeID).contents().find('head').append('<style>#ut-home-img {background: url(/courses/' + coursenum + '/file_contents/course%20files/global/css/images/homePageBanner.jpg) no-repeat center center; }</style>')
			}
		}
	...
	// Create an accordion in right panel after tools are added
		function initializeToolsAccordion(){
			...
			if($(iframeID).contents().find("#ut-modules-grid").length>0){
...
///////////////////////////////
		...
		function themeTool(){
			...
			if($("a[title='UT Template Wizard']").length>0){
				...
				$(".externalToolTrigger").click(function (e){
					e.preventDefault();
					var externalToolId = $("a[title='UT Template Wizard']").attr("id");
					tinyMCE.get("wiki_page_body").controlManager.get(externalToolId).settings.onclick();
				});
			} else {
				$(".themesContent").append('<div class="alert alert-info" style="margin-bottom: 5px;">Click the <img src="/images/downtick.png"> icon on the editor toolbar and choose "UT Template Wizard" to change the banner image.</div>');
			}
			outputThemes(nogridThemeArray);
			outputThemes(gridThemeArray);
			themesReady();
	...
	////// Base html variables for custom templates  //////
		var templateShell = '<div id="ut-template-front" class="horiz-nav-template"></div>';
	...
	////// Supporting functions  //////
		...
		// and is the first item on the page. Also replaces old css code
			...
			function templateChange(){
				$(".template-theme").click(function (e){
					e.preventDefault();
					templateCheck();
					// scrollToElement("#ut-template-front");
					// add the class for the selected template to the ut-template-front
						var templateClass = $(this).attr("rel");
						// console.log("I got here "+ templateClass);
						$(iframeID).contents().find("#ut-template-front").removeClass().addClass(templateClass);
							// themeElements(templateClass);
						if($(iframeID).contents().find("#ut-template-front").hasClass(templateClass)){
		...
		// If template is not there add it, will also remove and change old template
			function templateCheck(){
				addStyletoFrontIframe();
				var $contents = $(iframeID).contents();
				if($contents.find("#ut-template-front").length>0){
					var currentTheme = $contents.find("#ut-template-front").attr("class");
		...
		// Check and mark current theme in toolbar when the page loads
			function currentThemeCheck(){
				if($(iframeID).contents().find("#ut-template-front").length>0){
					var currentTheme = $(iframeID).contents().find("#ut-template-front").attr("class");
					$("#"+currentTheme).addClass("active-fp-theme");
					setTimeout(function(){
			...
			function themeSupportCheck(templateClass){
				...
				$.ajax({
				    ...
				    error:function (xhr, ajaxOptions, thrownError){
					    if(xhr.status==404) {
					        $(iframeID).contents().find('#ut-home-img').addClass('placeholder');
					        $(".externalToolTrigger").html('<i class="fa fa-picture-o"></i> Add Custom Banner');
					    } else {
					    	$(iframeID).contents().find('#ut-home-img').removeClass('placeholder');
...
///////////////////////////////
	...
	////// Base html variables for sections  //////
		var bannerSection = '<div id="ut-template-banner" class="title_banner">' +
			...
			'</div>';
		var imageSection = '<div id="ut-home-img" class="banner_image">&nbsp;</div>';
		var navSection = '<div id="ut-home-nav" class="navigation">' +
			'<ul>' +
			...
			'</ul>' +
			'</div>';
		var modulesSection = '<div id="ut-modules-grid" class="modules_grid">&nbsp;</div>';
		var footerSection = '<div class="contact_footer ut-home-footer">First Last, Ph.D. &nbsp;| &nbsp;first.last@utexas.edu&nbsp; | &nbsp;Department<br /><span class="ut-copyright">Materials in this course may be copyright protected. Please do not distribute them without permission of the copyright holder.</span></div>';
		var attributionSection = '<div class="image_attribution ut-home-attrib">Image attribution here.</div>';
		var socialMediaSection = '<div class="social_media ut-social">&nbsp;</div>';
	...
	////// Supporting functions  //////
		// If the checked section exists, move it, if not add it
			function checkSection(sectionName, frontPageSections){
				var $contents = $(iframeID).contents();
				var container = $contents.find("#ut-template-front");
				if($contents.find("."+sectionName).length>0){
                    $contents.find("."+sectionName).appendTo(container).removeClass("toRemove");
                    checkRemove();
				} else {
					$contents.find("#ut-template-front").append(frontPageSections[sectionName]);
				}
			}
		// This function loops through existing content and then updates section controls
			function identifySections(frontPageSections){
				...
				$contents.find("#ut-template-front").children("div").each(function(){
					if($(this+'[class]')){
...
///////////////////////////////
	...
	////// On Ready/Click functions  //////
		function updateNavItems(){
			$("#navListItems").html("");
			showHelp = false;
			$(iframeID).contents().find("#ut-home-nav li").each(function (i){
			...
			$("#navListItems").sortable({
				update: function( event, ui ) {
					...
					$('#navListItems li').each(function (i) {
						var connectedItem = $(this).attr('rel');
						$(iframeID).contents().find("#ut-home-nav ul").append($(iframeID).contents().find('li'+connectedItem));
					});
					updateNavItems();
				}
			});
		...
		function navItemReady(){
			$(".addNavItem").click(function (e){
				e.preventDefault();
				$(iframeID).contents().find("#ut-home-nav ul").append("<li>New Item</li>");
				checkNavCount();
				updateNavItems();
			});
		...
		function checkNavCount(){
			navItemCount = $(iframeID).contents().find("#ut-home-nav li").length;
			if(navItemCount < 4){
				$(".addNavItem").show();
			} else {
				$(".addNavItem").hide();
...
///////////////////////////////
	...
	//// On Ready/Click functions  //////
		function moduleListToolReady(){
			...
			$(".removeCurrentHighlight").click(function (e){
				e.preventDefault();
				$(iframeID).contents().find("#ut-modules-grid .current").removeClass("current");
	...
	////// Supporting functions  //////
			...
			function insertModuleList(){
				...
				$contents.find("#ut-modules-grid").html('<div id="#ut-modules-grid" />');
				$contents.find("#ut-modules-grid").load('/courses/'+coursenum+'/modules #context_modules', function() {
			...
			function markCurrent(){
				$(".mt-markCurrent").click(function (e){
					...
					$(iframeID).contents().find("#ut-modules-grid .current").removeClass("current");
					// $(iframeID).contents().find("#ut-modules-grid .icon-check").addClass("icon-standards").removeClass("icon-check");
			...
			function identifyModuleList(){
				if($(iframeID).contents().find("#ut-modules-grid").length>0){
					$(iframeID).contents().find("#ut-modules-grid li").each(function (){
						var isCurrent = "";
...
///////////////////////////////
	...
	////////// Custom Tools Accordion tab setup  //////  //////
		function socialMediaTool(){
			...
			$(".updateSocialMediaLinks").click(function (e){
				e.preventDefault();
				$(iframeID).contents().find(".ut-social").html("");
				$("#socialMediaLinks div").each(function (){
					...
					if(cleanedhref !== ""){
						...
						$(iframeID).contents().find(".ut-social").append('<a href="'+ newhref +'" class="'+ smIcon +'" title="'+newhref+'">&nbsp;</a>');
					}
				});
			});
	...
	////// On Ready/Click functions  //////
		function socialMediaReady(){
			$("iframe").contents().find(".ut-social").find("a").each(function(){
		...
		function changeIcon(){
			$(".changeIcon").click(function(){
				...
				if($(".iconElement").hasClass("active")){
					...
					$("iframe").contents().find("#ut-modules-grid a").each(function (){
